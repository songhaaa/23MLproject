...
def ssl_process(Net, X_train, X_test, Y_train, Y_test, n_labeled_per_class, random_seed,subject_num, session_num):
    ...
    result = train(Net, train_dataset_labeled, train_dataset_unlabeled, test_dataset, max_iterations, subject_num, session_num)
    result = np.squeeze(result) if result.ndim == 2 else result

    return result #shape = (30,)
...
if __name__ == '__main__':
    ...
    for seed in pbar_seed:
        ...
        if args.dataset=='SEED':
            ...
            print("acc_array[0][1]:", acc_array[0][1])
            print("acc_array[1][1]:", acc_array[1][1])
            new_acc_array = [[0]*3]*15
            for i in range(len(acc_array)):
                for j in range(len(acc_array[i])):
                    new_acc_array[i][j] = sum(acc_array[i][j]) / len(acc_array[i][j])

            print(np.shape(new_acc_array)) # ()
            print("new_acc_array[0][1]:", new_acc_array[0][1])
            print("new_acc_array[1][1]:", new_acc_array[1][1])
            # np.savetxt(os.path.join(directory, 'acc_labeled_{}.csv').format(n_labeled_per_class), acc_array[-1] , delimiter=",")
            np.savetxt(os.path.join(directory, 'acc_labeled_{}.csv').format(n_labeled_per_class), new_acc_array, delimiter=",")
