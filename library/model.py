...
class Conv_EEG(nn.Module):
    ...
    def forward(self, input):
        ...
        if grl_flag==True:
            ...
            # print(f"output_c : {output[0].size()}   |  output_d : {output[1].size()}")
            # print("grl output shape:", np.shape(output)) # 이게 출력이 안되고 에러가 걸린걸 보면 여기로 넘어오는 듯!
            ...
            # print("ae output shape:", np.shape(output))

        else:
            output = self.classifier(flatten_embed)
            # print("else output shape:", np.shape(output))


        return encoded_embed, output
        # return output
