...
# from library.model import Predictor_deep, Predictor_modified


def loss_unl(backbone, input_w, input_s, proto_s, args):
    '''The proposed losses for unlabeled target samples

    Parameters:
        net_G (network)    --The backbone
        net_F (network)    --The classifier (fc-l2norm-fc)
        input_w (tensor) --Weakly augmented inputs
        input_s (tensor) --Strongly augmented inputs
        proto_s (tensor)   --Source prototypes

    Return the three losses
    '''
    # forward
    # feat_tu = net_G(torch.cat((input_w, input_s), dim=0)) #여기서 문제가 생기는듯
    # feat_tu_con, logits_tu = net_F(feat_tu)

    # net_F = Predictor_modified(num_class=args.num_classes, inc=310, temp=args.T).to('cuda:0')
    # feat_tu, output = backbone(torch.cat((input_w, input_s), dim=0))  # 여기서 문제가 생기는듯
    # feat_tu_con, tmp = net_F(output, feat_tu)
    # logits_tu, disc_tu = torch.split(tmp, tmp.size(0) // 2, dim=0)
    ...
    pseudo_label = torch.softmax(logits_tu_w.detach() * args.T, dim=1) # args.T2 -> args.T로 수정
    ...
    return L_ot, L_con_cls, L_pl, consis_mask, feat_tu_w
...
def ot_loss(proto_s, feat_tu_w, feat_tu_s, args):
    ...
    with torch.no_grad():
        temp_feat_tu_w = feat_tu_w.clone()
        flatten = nn.Flatten()
        out_feat_tu_w = flatten(temp_feat_tu_w)
        # print(f"proto_s.mo_pro : {proto_s.mo_pro.size()}   |  out_feat_tu_w : {out_feat_tu_w.size()}")
        M_st_weak = 1 - pairwise_cosine_sim(proto_s.mo_pro, out_feat_tu_w)  # postive distance [bc, 306 * 10], [bc, 306 * 10]
        M_st_weak = 1 - pairwise_cosine_sim(proto_s.mo_pro, out_feat_tu_w)  # postive distance [bc, 306 * 10], [bc, 306 * 10]
...
def center_loss_cls(centers, x, labels, num_classes=65):
    ...
    flatten = nn.Flatten()
    out_x = flatten(x)
    # print(f"out_x : {out_x.size()}   |  centers_norm : {centers_norm.size()}")
    distmat = -out_x @ centers_norm.t() + 1
...
class Prototype:
    def __init__(self, C=4, dim=306, m=0.9):
    ...
    def update(self, feats, lbls, norm=False):
        b, h, w = feats.shape
        feats = feats.view(b, h * w)
        momentum = self.m
        for i_cls in torch.unique(lbls):
            feats_i = feats[lbls.argmax(1) == i_cls, :]
            feats_i_center = feats_i.mean(dim=0, keepdim=True)
            self.mo_pro[int(i_cls), :] = self.mo_pro[int(i_cls), :] * \
                momentum + feats_i_center * (1 - momentum)
            self.batch_pro[int(i_cls), :] = feats_i_center
        if norm:
            self.mo_pro = F.normalize(self.mo_pro)
            self.batch_pro = F.normalize(self.batch_pro)
